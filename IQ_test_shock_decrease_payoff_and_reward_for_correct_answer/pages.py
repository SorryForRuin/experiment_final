...
class Shock(Page):
    ...
    def vars_for_template(self):
        return {
            "decrease_in_payment":Constants.decrease_in_payment,
            "payment_per_correct_answer": Constants.payment_per_correct_answer
        }
class IQ_test1(Page):
    form_model = "player"
...
class Results(Page):
    ...
    def vars_for_template(self):
        ...
        if self.player.rank <= 5:
            self.player.payoff1 = Constants.fix_payment - Constants.decrease_in_payment + Constants.payment_per_correct_answer * self.player.combined_score - Constants.variable_payment_coef * (1 - (self.player.likelihood1 / 100)) * (1 - (self.player.likelihood1 / 100))
            self.player.payoff2 = Constants.fix_payment - Constants.decrease_in_payment + Constants.payment_per_correct_answer * self.player.combined_score - Constants.variable_payment_coef * (1 - (self.player.likelihood2 / 100)) * (1 - (self.player.likelihood2 / 100))
        else:
            self.player.payoff1 = Constants.fix_payment - Constants.decrease_in_payment + Constants.payment_per_correct_answer * self.player.combined_score - Constants.variable_payment_coef * (0 - (self.player.likelihood1 / 100)) * (0 - (self.player.likelihood1 / 100))
            self.player.payoff2 = Constants.fix_payment - Constants.decrease_in_payment + Constants.payment_per_correct_answer * self.player.combined_score - Constants.variable_payment_coef * (0 - (self.player.likelihood2 / 100)) * (0 - (self.player.likelihood2 / 100))
...
class Confirm(Page):
    ...
    # def ConfirmQ1_error_message(player, value):
    #
    #     if q1:
...
page_sequence = [Instructions, IQ_test1, IQ_test2, IQ_test3, IQ_test4, IQ_test5, IQ_test6,
                 ...
                 Likelihood1, Shock, Feedback, Confirm, Likelihood2, Results,  SurveyQuestions]
